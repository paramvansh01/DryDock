...
|---|---|---|
| **0:00–0:28** Problem | Title card "DRYDOCK — safe writes for AI agents", then the **Overview** tab. | "A bank acquires a smaller one. Two customer databases, the same people in both, spelled differently. Someone asks an AI agent to merge them. It writes the SQL. It runs. And a father and son living at the same address become one customer — one loses his loan history, the other inherits it. Nobody notices for weeks, because nobody could see which rows would change before it ran. That's why Exasol's own MCP server ships with writes switched off." |
...
| **2:30–2:48** Results | **Runs** scoreboard, or the README's *Results on the live instance* table. | "The results: 27,980 of 27,981 proposed merges correct, 99.9% recall, and all 600 duplicates found. Every score is measured against a ground truth the agent never sees, and every change passed through the gate." |
